...
function QueryStringParser(req) {
    ...
    function toggleCaptureButton() {
        captureTraffic == $("#capture").toggleClass("active");
        showInfo(selectedInfo);
    }

    function toggleRawViewButton() {
        !settings.view_Raw == $("#rawView").toggleClass("active");
        showInfo(selectedInfo);
    }
...
function saveRaw() {
    var n = "",
        r, u;
    for (i = 0; i < headerInfo.response.length; i++) {
        headerInfo.request[i].requestHeaders.sort(sortHeaders);
        headerInfo.response[i].responseHeaders.sort(sortHeaders);
        var e = headerInfo.response[i].statusLine.split(" "),
            t = parseURL(headerInfo.response[i].url),
            f = t.host;
        for (t.port != undefined ? f += ":" + t.port : t.scheme == "https" && (f += ":443"), n += headerInfo.request[i].method + " " + t.path + " " + e[0] + "\n", n += "Host: " + f + "\n", x = 0; x < headerInfo.request[i].requestHeaders.length; x++) r = headerInfo.request[i].requestHeaders[x].name, u = headerInfo.request[i].requestHeaders[x].value, n += r + ": ", n += u + "\n";
        for (n += "\n", n += headerInfo.response[i].statusLine + "\n", x = 0; x < headerInfo.response[i].responseHeaders.length; x++) r = headerInfo.response[i].responseHeaders[x].name, u = headerInfo.response[i].responseHeaders[x].value, n += r + ": ", n += u + "\n";
        n += "\n"
    }
    console.log (n);
    // var mainTable = $('#mainTable').clone();
    // $("#exportAsText").html($('#previewArea').html()).find('.results.preview').css({"height":"auto","width":"auto"})
    $("#exportAsText").html(n)
}
